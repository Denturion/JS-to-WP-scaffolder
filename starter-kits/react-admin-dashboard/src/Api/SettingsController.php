 ...
 *
 * @package ReactAdminDashboard\Api
 */

namespace ReactAdminDashboard\Api;

use ReactAdminDashboard\Models\Settings;
 ...
 *   GET  /wp-json/react-admin-dashboard/v1/settings
 *   POST /wp-json/react-admin-dashboard/v1/settings  { body: JSON.stringify(settings) }
 */
class SettingsController {
	 ...
	 * @var string
	 */
	private string $namespace = 'react-admin-dashboard/v1';
	...
	public function update_settings( \WP_REST_Request $request ) {
		...
		if ( empty( $params ) ) {
			return new \WP_Error(
				'rest_invalid_param',
				__( 'Request body must be a JSON object.', 'react-admin-dashboard' ),
				[ 'status' => 400 ]
			);
		}

		$updates = [];
		if ( array_key_exists( 'dashboard_title', $params ) ) {
			$updates['dashboard_title'] = $params['dashboard_title'];
		}
		if ( array_key_exists( 'primary_color', $params ) ) {
			$updates['primary_color'] = $params['primary_color'];
		}
		if ( array_key_exists( 'items_per_page', $params ) ) {
			$updates['items_per_page'] = $params['items_per_page'];
		}
	...
	public function check_permission() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new \WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to manage plugin settings.', 'react-admin-dashboard' ),
				[ 'status' => 403 ]
			);
		}
	...
	private function get_endpoint_args(): array {
		return [
			...
			'primary_color' => [
				...
				'sanitize_callback' => 'sanitize_text_field',
				'validate_callback' => 'rest_validate_request_arg',
				'description'       => 'Primary Color (Tailwind class, e.g. blue-600)',
			],
			'items_per_page' => [
				'type'              => 'number',
				'required'          => false,
