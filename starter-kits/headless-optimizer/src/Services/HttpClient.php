 ...
 * HTTP Client Service
 *
 * @package HeadlessOptimizer\Services
 */

namespace HeadlessOptimizer\Services;
...
class HttpClient {
	...
	private function parse_response( $response ): ?array {
		...
		if ( is_wp_error( $response ) ) {
			error_log(
				sprintf(
					'[HeadlessOptimizer] HttpClient error: %s',
		...
		if ( $status_code < 200 || $status_code >= 300 ) {
			error_log(
				sprintf(
					'[HeadlessOptimizer] HttpClient received non-2xx status: %d',
					$status_code
				)
			);
			return null;
